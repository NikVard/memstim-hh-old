...
if __name__ == "__main__":
    ...
    t_lims = [1500*ms, 6500*ms] # ms : x-axs limits
    ...
    # Use gridspecs
    G_outer_figure = fig.add_gridspec(2, 2, left=0.075, right=0.925, bottom=0.075, top=0.925,
                                            wspace=0.2, hspace=0.5) # these are not working!!!
    ...
    fig.suptitle('Phase Reset Enabled                                             Phase Reset Disabled', horizontalalignment='center', fontsize=fsize_panels)
    ...
    for G_curr, panel_label, results_dir_curr, t_stim in zip(G_outer_figure, ['A.', 'B.', 'C.', 'D.'], analysis_dirs, [1850.3*ms, 1850.3*ms, 1934.4*ms, 1934.4*ms]):
        print('[*] Panel', panel_label)

        # Adjust limits
        t_lims_adj = [t_stim + 2050*ms, t_stim+4050*ms] # ms : calculate mean FRs in a 2-sec window
        duration_adj = t_lims_adj[1] - t_lims_adj[0]

        # Use gridspecs
        G_outer = GridSpecFromSubplotSpec(3, 1, height_ratios=(0.1, 0.1, 0.8), subplot_spec=G_curr)
