...
if __name__ == "__main__":
    ...
    t_lims = [1250*ms, 6500*ms] # ms : x-axs limits
    ...
    # Use gridspecs
    G_outer_figure = fig.add_gridspec(4, 3, left=0.01, right=0.95, bottom=0.02, top=0.995,
                                            height_ratios=(0.01, 0.395, 0.395, 0.2),
    ...
    G_quantify = G_outer_figure[3,1:].subgridspec(1,3, wspace=0.0)
    ...
    for G_curr, panel_label, results_dir_curr, t_stim in zip(G_outer_figure_nb, ['A.', 'B.', 'C.', 'D.'], analysis_dirs, [1850.3*ms, 1850.3*ms, 1934.4*ms, 1934.4*ms]):
        ...
        if idx_all.size > 1:
            ...
            for idx in idx_all[1:]:
                if idx - curr_idx > 1:
                    ax_rhythm.scatter(x=tv[idx], y=1.25, s=25, marker='o', edgecolors='white', facecolors='gray', rasterized=False, clip_on=False)
                curr_idx = idx
    ...
    for G_curr, data, cmap_curr, label in zip(G_quantify, [theta_band_power, gamma_band_power, PAC_metric], [mplb.cm.inferno, mplb.cm.inferno, mplb.cm.viridis], ["CA1 Theta Power", "CA1 Gamma Power", "CA1 Phase-Amplitude Coupling"]):

        # Add an axis
        ax = fig.add_subplot(G_curr)
        axs_quantify.append(ax)

        # list to array
        data_arr = np.array(data)

        # Colormap normalization
        img_data = data_arr[3::4,0].reshape(2,2)
        norm_cmap = mplb.colors.Normalize(vmin=0, vmax=np.round(img_data.max()))

        # Use imshow to show the matrix
        img = ax.imshow(img_data, cmap=cmap_curr, norm=norm_cmap, extent=[-1,1,-1,1])
        ...
        cbar = fig.colorbar(img, cax=cax, ticks=np.round(np.linspace(0., img_data.max(), 3),1), orientation='horizontal')
        ...
        # Add texts
        ax.text(s=label, weight='bold', x=0., y=1.5, fontsize=fsize_titles, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Phase Reset', x=0., y=1.25, fontsize=fsize_misc, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Enabled', x=-0.5, y=1.1, fontsize=fsize_xylabels, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Disabled', x=0.5, y=1.1, fontsize=fsize_xylabels, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Stimulation', x=-1.25, y=0, fontsize=fsize_misc, rotation=90, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Peak', x=-1.1, y=0.5, fontsize=fsize_xylabels, rotation=90, transform=ax.transData, va='center', ha='center', clip_on=False)
        ax.text(s='Trough', x=-1.1, y=-0.5, fontsize=fsize_xylabels, rotation=90, transform=ax.transData, va='center', ha='center', clip_on=False)
    ...
    axs_quantify[0].text(x=-1.07, y=1.2, s='E.', fontsize=fsize_panels, weight='bold', ha='left', va='top', transform=axs_quantify[0].transAxes)
