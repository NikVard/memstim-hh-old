...
tv = np.arange(0, T, dt)
stim_on = 1.
Twin = 0.5 # 500ms window to animate, around the stimulation pulse
...
EC_exc_raster_trim = EC_exc_raster[twin_idx-1000:twin_idx+4000]
EC_inh_raster_trim = EC_inh_raster[twin_idx-1000:twin_idx+4000]
DG_exc_raster_trim = DG_exc_raster[twin_idx-1000:twin_idx+4000]
DG_inh_raster_trim = DG_inh_raster[twin_idx-1000:twin_idx+4000]
CA3_exc_raster_trim = CA3_exc_raster[twin_idx-1000:twin_idx+4000]
CA3_inh_raster_trim = CA3_inh_raster[twin_idx-1000:twin_idx+4000]
CA1_exc_raster_trim = CA1_exc_raster[twin_idx-1000:twin_idx+4000]
CA1_inh_raster_trim = CA1_inh_raster[twin_idx-1000:twin_idx+4000]
...
EC_exc_scat = ax3D.scatter(EC_exc_pos[:,0], EC_exc_pos[:,1], EC_exc_pos[:,2], s=1., edgecolors=edges, facecolors=faces_pyCAN, cmap='hot', vmin=0., vmax=1.)
EC_inh_scat = ax3D.scatter(EC_inh_pos[:,0], EC_inh_pos[:,1], EC_inh_pos[:,2], s=1., edgecolors=edges, facecolors=faces_inh, cmap='hot', vmin=0., vmax=1.)
DG_exc_scat = ax3D.scatter(DG_exc_pos[:,0], DG_exc_pos[:,1], EC_exc_pos[:,2], s=1., edgecolors=edges, facecolors=faces_py, cmap='hot', vmin=0., vmax=1.)
DG_inh_scat = ax3D.scatter(DG_inh_pos[:,0], DG_inh_pos[:,1], DG_inh_pos[:,2], s=1., edgecolors=edges, facecolors=faces_inh, cmap='hot', vmin=0., vmax=1.)
CA3_exc_scat = ax3D.scatter(CA3_exc_pos[:,0], CA3_exc_pos[:,1], CA3_exc_pos[:,2], s=1., edgecolors=edges, facecolors=faces_pyCAN, cmap='hot', vmin=0., vmax=1.)
CA3_inh_scat = ax3D.scatter(CA3_inh_pos[:,0], CA3_inh_pos[:,1], CA3_inh_pos[:,2], s=1., edgecolors=edges, facecolors=faces_inh, cmap='hot', vmin=0., vmax=1.)
CA1_exc_scat = ax3D.scatter(CA1_exc_pos[:,0], CA1_exc_pos[:,1], CA1_exc_pos[:,2], s=1., edgecolors=edges, facecolors=faces_pyCAN, cmap='hot', vmin=0., vmax=1.)
CA1_inh_scat = ax3D.scatter(CA1_inh_pos[:,0], CA1_inh_pos[:,1], CA1_inh_pos[:,2], s=1., edgecolors=edges, facecolors=faces_inh, cmap='hot', vmin=0., vmax=1.)

# Text label to indicate time
# ttext = ax3D.text(20,10,120,'time=0',fontsize=10)
...
def init():
    ...
    # EC_exc_scat = ax3D.scatter(EC_exc_pos[:,0], EC_exc_pos[:,1], EC_exc_pos[:,2], color=[0.6,0.6,0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # EC_inh_scat= ax3D.scatter(EC_inh_pos[:,0], EC_inh_pos[:,1], EC_inh_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # DG_exc_scat = ax3D.scatter(DG_exc_pos[:,0], DG_exc_pos[:,1], DG_exc_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # DG_inh_scat = ax3D.scatter(DG_inh_pos[:,0], DG_inh_pos[:,1], DG_inh_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # CA3_exc_scat = ax3D.scatter(CA3_exc_pos[:,0], CA3_exc_pos[:,1], CA3_exc_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # CA3_inh_scat = ax3D.scatter(CA3_inh_pos[:,0], CA3_inh_pos[:,1], CA3_inh_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # CA1_exc_scat = ax3D.scatter(CA1_exc_pos[:,0], CA1_exc_pos[:,1], CA1_exc_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)
    # CA1_inh_scat = ax3D.scatter(CA1_inh_pos[:,0], CA1_inh_pos[:,1], CA1_inh_pos[:,2], color=[0.6, 0.6, 0.6], cmap='hot', vmin=0., vmax=1., alpha=0.25)


    # set camera view
    # ax3D.view_init(elev=54., azim=-145)
...
def animate(i):
    ...
    # Update text every 30 frames TODO: change this to ms
    if (i % 30 == 0):
        ctime = int(tidx*dt)*1000 # msec
        ttl.set_text('Time = {}ms'.format(ctime))

    # update camera view
    ax3D.view_init(elev=54., azim=-145+0.05*i)
...
step = 5 # step over X samples per frame
F = twin_idx // step # total number of frames
tidx = int((stim_on-0.005) // dt)
tidx = int(stim_on // dt)
fig.canvas.draw()

print(">> Starting animation...")
movie = animation.FuncAnimation(fig, animate, frames=120, interval=20, blit=True, init_func=init)
