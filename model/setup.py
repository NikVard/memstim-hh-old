    ...
    for pypop in range(N_py): # iterate over pyramidal populations in the current area
        ...
        for inhpop in range(N_inh):
            if p_conn[pypop][N_py+inhpop]!=0. and G_py[pypop] and G_inh[inhpop]: # connection is valid (p>0); exc pop exists; inh pop exists. # the addition at the start makes sure we go to the inhibitory section of the connections
                ...
                syn_EI.connect(condition='i!=j', p=str(p_conn[pypop][N_py+inhpop])) #+'*exp(-((x_soma_pre-x_soma_post)**2+(y_soma_pre-y_soma_post)**2+(z_soma_pre-z_soma_post)**2)/(2*(2500*umetre)**2))') # connect E2I
            ...
            if p_conn[N_py+inhpop][pypop]!=0 and G_inh[inhpop] and G_py[pypop]: # same as before, but the other way around, I2E connection
                ...
                syn_IE.connect(condition='i!=j', p=str(p_conn[N_py+inhpop][pypop])) #+'*exp(-((x_soma_pre-x_soma_post)**2+(y_soma_pre-y_soma_post)**2+(z_soma_pre-z_soma_post)**2)/(2*(350*umetre)**2))') # connect I2E
        ...
        if G_py_from:
            ...
            for destination in range(N_py_to):  # from Py (origin) to Py (destination)
                ...
                if G_py_to:
                    ...
                    syn_E.connect(condition="i!=j", p=str(all_p[origin][destination])) #+'*exp(-((y_soma_pre-y_soma_post)**2)/(2*(1000*umetre)**2))')
            ...
            for destination in range(N_inh_to): # from Py (origin) to Inh (destination)
                ...
                if G_inh_to:
                    ...
                    syn_I.connect(condition="i!=j", p=str(all_p[origin][N_py_to+destination])) #+'*exp(-((y_soma_pre-y_soma_post)**2)/(2*(1000*umetre)**2))')
