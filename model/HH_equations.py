...
py_CAN_eqs = '''
    ...
    I_CAN =  ((gCAN) * (size)) * mCAN ** 2 * (v - ECAN) : amp
    ...
    I_K = ((5 * msiemens * cm ** -2) * (size)) * (n ** 4) * (v - (-100*mV)) : amp
    ...
    noise = sigma_noise_exc*(2*(0.1e-3 * siemens ) / (1 * ufarad))**.5*randn()/sqrt(pas_de_temps) : volt/second (constant over dt)
...
""" ------------------------------------------------------------------------ """
inh_eqs = '''
    dv/dt = ( - I_leak - I_K - I_Na - I_SynE - I_SynExt - I_SynHipp - I_SynI + r*I_stim) / ((1 * ufarad * cm ** -2) * (taille)) + noise: volt
    ...
    noise = sigma_noise_inh*(2*(0.1e-3 * siemens ) / (1 * ufarad))**.5*randn()/sqrt(pas_de_temps) : volt/second (constant over dt)
